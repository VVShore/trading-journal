"""
Trading Journal — Whop Hosted Version (fixed)
Fixes:
  - X-Frame-Options and CSP headers so Whop iframe works
  - Neon SSL connection handled correctly
  - /health endpoint for Railway
  - Connection pool handles dropped connections gracefully
"""
...
# CORS — allow all origins so the iframe and direct access both work
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# This is the core fix for Whop iframe embedding.
# We explicitly REMOVE X-Frame-Options and set a permissive CSP.
# Without this, browsers block the iframe with "refused to connect" errors.
class EmbedHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        response = await call_next(request)

        # Allow embedding in any iframe (required for Whop)
        # Removing X-Frame-Options entirely is safer than setting ALLOWALL
        # because some browsers treat ALLOWALL inconsistently
        if "x-frame-options" in response.headers:
            del response.headers["x-frame-options"]
        if "X-Frame-Options" in response.headers:
            del response.headers["X-Frame-Options"]

        # Content Security Policy — allow Whop to embed us
        # frame-ancestors * means any site can embed this page
        ...
        # Required for cross-origin cookies and storage in iframes
...
def _build_dsn(url: str) -> str:
    """
    Neon requires SSL. If the URL doesn't already include sslmode,
    append it. psycopg2 needs sslmode=require for Neon connections.
    """
    if not url:
        return url
...
def get_pool() -> Optional[ThreadedConnectionPool]:
    ...
    if _pool is None and DATABASE_URL:
        ...
        _pool = ThreadedConnectionPool(
            ...
            maxconn=10,
            dsn=dsn,
            # keepalives prevent Neon from closing idle connections
            keepalives=1,
            keepalives_idle=30,
...
def db():
    ...
    try:
        conn = pool.getconn()
        # Test connection — Neon closes idle connections after 5 minutes
        # If broken, get a fresh one
        try:
            cur = conn.cursor()
            cur.execute("SELECT 1")
            cur.close()
...
def ai(prompt: str, max_tokens: int = 120) -> str:
    if not _groq:
        return "Add GROQ_API_KEY to Railway environment variables to enable AI analysis."
    try:
...
def _serialize(obj):
    """Make datetime/date objects JSON-safe"""
...
def today_stats(uid: str = Depends(current_user)):
    ...
    q = {"A":0,"B":0,"C":0}
    for t in trades:
        if t.get("quality") in q: q[t["quality"]] += 1

    last_result = None; consecutive_losses = 0
    ...
    return {
        ...
        "session": session,
        "guardrails": {
            "over_2":    total >= 2, "over_3": total >= 3, "over_am": am >= 2,
            "tilt":      any(t["state"] == "tilt" for t in trades),
            "post_loss": last_result == "loss", "post_2loss": consecutive_losses >= 2,
...
def close_session(uid: str = Depends(current_user)):
    ...
    if post_loss: lines.append(f"After loss: averaged {sum(post_loss)/len(post_loss):+.2f}R next trade.")
    if total > 2: lines.append(f"{total} trades — above 2-trade structure.")
    if q["C"] == 0 and total <= 2: lines.append(f"Clean — {total} trade{'s' if total>1 else ''}, no C-setups.")
    ...
    signal = signals.get(biggest_leak,"One trade at a time.") if grade != "A" else "That's the standard. Replicate the state."
    ...
    with db() as conn:
        ...
        """, (
            uid,grade,craftsman,dominant,biggest_leak,pattern,signal,total_r,win_rate,total,mirror,
            grade,craftsman,dominant,biggest_leak,pattern,signal,total_r,win_rate,total,mirror,
        ))
        cur.execute(
...
def _gen_mirror(r_trades, grade, biggest_leak, dominant, q):
    ...
    prompt = (f"Session: {', '.join(ctx)}. Grade:{grade}.\n"
...
def analytics(days: int = 30, uid: str = Depends(current_user)):
    with db() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT * FROM trades WHERE user_id=%s AND timestamp >= NOW() - INTERVAL '%s days' ORDER BY timestamp",
            (uid, days)
        )
        trades = _serialize(rows(cur))
        cur.execute(
    ...
    edge = {"text":"Log more A-quality trades to define your edge.","ready":False} if a["total"]<3 else {
        "text": f"A-quality: {a['wr']}% win rate · {'+' if a['total_r']>=0 else ''}{a['total_r']}R total." + (f"  C-setups: {c['total_r']}R cost." if c["total"]>0 else ""),
        "ready": True
    }
    ...
    def leak(key,bad):
        bg=[t for t in r_trades if t.get(key) in bad]; gd=[t for t in r_trades if t.get(key) and t.get(key) not in bad]
        return {"bad_avg_r":round(sum(t["r_multiple"] for t in bg)/len(bg),2) if bg else None,
                "good_avg_r":round(sum(t["r_multiple"] for t in gd)/len(gd),2) if gd else None,
                "bad_n":len(bg),"good_n":len(gd)}

    s_t=sorted(r_trades,key=lambda t:t["timestamp"]); wal={"total":0,"wins":0}; cal={"total":0,"c":0}
    for i,t in enumerate(s_t):
        if i==0: continue
        prev=s_t[i-1]; same=t["timestamp"][:10]==prev["timestamp"][:10]
    ...
    daily={}
    for t in s_t:
        d=t["timestamp"][:10]; daily[d]=round(daily.get(d,0)+t["r_multiple"],2)
    cum=0; cum_r=[]
    ...
    return {
        ...
        "sequence":{"win_after_loss":round(wal["wins"]/wal["total"]*100,1) if wal["total"] else None,
                    "c_after_loss":round(cal["c"]/cal["total"]*100,1) if cal["total"] else None,"samples":wal["total"]},
...
def analyze(days: int = 7, uid: str = Depends(current_user)):
    ...
    total_r  = round(sum(t["r_multiple"] for t in r_trades),2)
    lines = [
        f"[{t['timestamp'][:10]}] #{t['session_num']} State:{t.get('state','?')} Q:{t.get('quality','?')} "
        f"Setup:{t.get('setup_type','?')} Entry:{t.get('entry_behavior','?')} R:{t.get('r_multiple','?')}"
        for t in trades
    ]
    prompt = (f"Analyze {days} days. Total R: {total_r:+.1f}.\n\n"+"\n".join(lines)+
...
def _bridge(command: str, extra: str = None, timeout: int = 22) -> dict:
    if not BRIDGE_SCRIPT.exists():
        return {"error": "chart_bridge.js not found."}
    args = ["node", str(BRIDGE_SCRIPT), command]
    if extra: args.append(extra)
    try:
        r   = subprocess.run(args, capture_output=True, text=True, timeout=timeout, cwd=str(BRIDGE_SCRIPT.parent))
