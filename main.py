"""
Trading Journal — Whop Hosted Version
PostgreSQL + Groq AI + Whop user isolation

Environment variables required (set in Railway dashboard):
  DATABASE_URL   — provided automatically by Railway PostgreSQL plugin
  WHOP_API_KEY   — from Whop dashboard > Developer Settings > API Keys
  JWT_SECRET     — generate with: python -c "import secrets; print(secrets.token_hex(32))"
  GROQ_API_KEY   — from console.groq.com (free account)

Optional:
  DEV_MODE       — set to "true" to bypass Whop auth during local testing
  DEV_USER_ID    — test user ID when DEV_MODE is true (default: "dev_user_001")
"""
...
WHOP_API_KEY = os.environ.get("WHOP_API_KEY", "")      # ← user fills in Railway
JWT_SECRET   = os.environ.get("JWT_SECRET", "insecure-default-change-me")  # ← user fills in Railway
GROQ_API_KEY = os.environ.get("GROQ_API_KEY", "")      # ← user fills in Railway
...
async def auth_whop(request: Request):
    """
    Exchange a Whop user token for a journal JWT.
    Called by the frontend after Whop passes ?token= in the URL.
    """
    if DEV_MODE:
        jwt = create_jwt(DEV_USER_ID)
        return {"jwt": jwt, "user_id": DEV_USER_ID}
    ...
    if resp.status_code != 200:
        raise HTTPException(401, "Invalid or expired Whop token — check your membership")
    ...
    jwt = create_jwt(str(user_id))
    return {"jwt": jwt, "user_id": str(user_id)}
...
def today_stats(uid: str = Depends(current_user)):
    with db() as conn:
        ...
        trades  = rows(cur)
        cur.execute(
            "SELECT * FROM sessions WHERE user_id=%s AND date=CURRENT_DATE",
            (uid,)
        )
        session    = row_one(cur)
...
def close_session(uid: str = Depends(current_user)):
    ...
    signal = signals.get(biggest_leak,"One trade at a time.") if grade != "A" else "That's the standard. Replicate the state, replicate the result."
    ...
    with db() as conn:
        ...
        """, (
            uid, grade, craftsman, dominant,
            biggest_leak, pattern, signal, total_r, win_rate, total, mirror,
            grade, craftsman, dominant,
            biggest_leak, pattern, signal, total_r, win_rate, total, mirror,
        ))
        cur.execute(
...
def analytics(days: int = 30, uid: str = Depends(current_user)):
    ...
    for s in sessions:
        for k, v in s.items():
            if isinstance(v, (datetime, date)): s[k] = v.isoformat()
...
def analyze(days: int = 7, uid: str = Depends(current_user)):
    ...
    prompt = (f"Analyze {days} days of trading. Total R: {total_r:+.1f}.\n\n" + "\n".join(lines) +
...
# ── Chart bridge (local-only) ─────────────────────────
def _bridge(command: str, extra: str = None, timeout: int = 22) -> dict:
    if not BRIDGE_SCRIPT.exists():
        return {"error": "chart_bridge.js not found. This feature requires local setup."}
